    ...
    <section className="relative flex min-h-screen h-[100svh] items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-[20rem] space-y-8 px-5 text-center md:max-w-4xl md:space-y-10 md:px-6">
        <div ref={r1} className={`reveal ${v1 ? 'visible' : ''} space-y-4 md:space-y-6`}>
          <p className="font-display text-[2rem] leading-tight text-white sm:text-4xl md:text-5xl" style={{ textShadow: '0 1px 4px hsl(220 20% 8% / 0.45)' }}>
            ชีวิตจริงที่
          </p>
          <p className="text-[3.75rem] leading-[1.08] text-white sm:text-6xl md:text-7xl" style={{ fontFamily: "'Quentin', 'Playfair Display', cursive", fontStyle: 'normal', textShadow: '0 1px 4px hsl(220 20% 8% / 0.45)' }}>
            Wonderful
          </p>
          <p className="font-display text-[2rem] leading-tight text-white sm:text-4xl md:text-5xl" style={{ textShadow: '0 1px 4px hsl(220 20% 8% / 0.45)' }}>
            ได้ทุกวัน
          </p>
          <p className="font-body text-base text-white/90 sm:text-xl md:text-2xl" style={{ textShadow: '0 1px 3px hsl(220 20% 8% / 0.4)' }}>
            ที่ <span className="tracking-[0.08em] text-white md:tracking-wider" style={{ textShadow: '0 1px 3px hsl(220 20% 8% / 0.4)' }}>คอนโด SUPALAI</span>
          </p>
        </div>
      </div>
    </section>
